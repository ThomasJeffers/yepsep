...
Kamailio S-CSCF (and any SIP proxy) matches INVITE transactions on the
*top* Via branch. Responses MUST copy every Via in order. Copying only
the last Via (the UE) makes S-CSCF ignore 200 OK and emit 408.
...
# RFC 3261 compact forms. Kamailio usually sends long names; map anyway.
SIP_COMPACT = {
    "v": "Via",
    "f": "From",
    "t": "To",
...
def canonical_header_name(key: str) -> str:
    stripped = key.strip()
    compact = SIP_COMPACT.get(stripped.lower())
    if compact:
        return compact
    return stripped
...
def parse_sip_message(data: bytes) -> dict:
    ...
    Multi-occurrence headers (Via, Record-Route, Route) are always lists,
    in on-the-wire order. First Via is the top/most-recent hop.
    """
    ...
    raw_lines = header_part.split("\n")
    start_line = raw_lines[0].strip() if raw_lines else ""
    headers: dict = {}
    ...
    for raw in raw_lines[1:]:
        ...
        if pending_key and raw[:1] in " \t":
            headers[pending_key][-1] = headers[pending_key][-1] + " " + raw.strip()
            continue
        if ":" not in raw:
            continue
        key, value = raw.split(":", 1)
        key = canonical_header_name(key)
        value = value.strip()
        if not key:
            continue
        pending_key = key
...
def get_headers(headers: dict, name: str) -> List[str]:
...
def get_header(headers: dict, name: str, default: str = "") -> str:
    """Return the first (top) occurrence. For Via that is the nearest hop."""
    values = get_headers(headers, name)
    return values[0] if values else default


def via_branch(via_value: str) -> str:
    m = re.search(r"(?:^|;)\s*branch=([^;>\s]+)", via_value or "", re.IGNORECASE)
    return m.group(1) if m else ""


def format_via_summary(headers: dict) -> str:
    vias = get_headers(headers, "Via")
    if not vias:
        return "via=0"
    top = vias[0]
    return f"via={len(vias)} top={top} branch={via_branch(top) or '-'}"


def render_sip_response(code: int, reason: str, headers: dict, body: str = "") -> bytes:
    """Build a SIP response on the wire. Multi-value headers (Via, Record-Route)
    are emitted as repeated fields in list order."""
...
def copy_request_headers_for_response(request_headers: dict, extra: Optional[dict] = None) -> dict:
    headers: dict = {
        "Via": get_headers(request_headers, "Via"),
        "Record-Route": get_headers(request_headers, "Record-Route"),
        ...
        "User-Agent": "MCPTT-Server-Mock/1.2",
    }
    if extra:
...
class MCPTTServer:
    ...
    def handle_sip(self, data: bytes, addr: Tuple[str, int]) -> None:
        try:
            ...
            logger.info(
                "SIP RX [%s] from %s:%s Call-ID=%s body=%sB %s",
                method,
                addr[0],
                addr[1],
                call_id or "-",
                len(body.strip()),
                format_via_summary(headers),
            )
    ...
    def response_headers(self, request_headers: dict, extra: Optional[dict] = None) -> dict:
        return copy_request_headers_for_response(request_headers, extra)
    ...
    def handle_invite(self, headers: dict, body: str, addr: Tuple[str, int]) -> None:
        ...
        with self.lock:
            ...
            if existing[0] is not None:
                ...
                self.send_sip_response(
                    ...
                    self.response_headers(
                        ...
                        {
                            "To": leg.to_header,
                            "Contact": f"<sip:{self.advertise_ip}:{self.sip_port};transport=udp>",
            ...
            if call_id in self.pending_invites:
                logger.info("INVITE Call-ID=%s already in flight — ignoring overlapping copy", call_id)
                return
            self.pending_invites.add(call_id)

        try:
            self._process_new_invite(headers, body, addr, call_id, from_hdr, to_hdr)
        finally:
            with self.lock:
                self.pending_invites.discard(call_id)

    def _process_new_invite(
        self,
        headers: dict,
        body: str,
        addr: Tuple[str, int],
    ...
    def send_sip_response(self, addr: Tuple[str, int], code: int, reason: str, headers: dict, body: str = "") -> None:
        msg = render_sip_response(code, reason, headers, body)
        try:
            self.sip_socket.sendto(msg, addr)
            logger.info(
                "SIP TX [%s %s] to %s:%s Call-ID=%s %s bytes=%s",
                code,
                reason,
                addr[0],
                addr[1],
                headers.get("Call-ID") or "-",
                format_via_summary(headers),
                len(msg),
            )
        except OSError as exc:
